...
DATA_FILE = "CAP_Seat_Allocation (v2).xlsx - CAP I - Maharashtra (MHTCET) (1).csv"
...
def simplify_category(cat):
    ...
    if 'NT 1' in cat or 'NT-1' in cat or 'NT-B' in cat: return 'NT-1'
    if 'NT 2' in cat or 'NT-2' in cat or 'NT-C' in cat: return 'NT-2'
    if 'NT 3' in cat or 'NT-3' in cat or 'NT-D' in cat: return 'NT-3'
    if 'DT' in cat or 'VJ' in cat: return 'VJ/DT'
    if 'SEBC' in cat: return 'SEBC'
    if 'SBC' in cat: return 'SBC'
    return 'OPEN'
...
def load_and_preprocess_data():
    try:
        ...
        df_clean = df.dropna(subset=['MHTCET Percentile', 'Gender', 'Category', 'Institute Name', 'Course Name']).copy()
...
CUTOFF_BENCHMARKS = pd.DataFrame({
    'Category': ['OPEN', 'SEBC', 'OBC', 'NT-3', 'NT-2', 'VJ/DT', 'SC', 'ST'],
    'Median Cutoff Percentile': [91.5, 88.9, 87.3, 82.1, 80.5, 78.4, 73.6, 62.0],
    'Seat Matrix Allocation (%)': [40.0, 10.0, 19.0, 3.5, 3.5, 3.0, 13.0, 7.0]
})
...
# ==========================================
with st.sidebar:
    st.image("https://cdn-icons-png.flaticon.com/512/3429/3429149.png", width=55)
    ...
    gender_input = st.radio("Gender Classification", options=["Female (F)", "Male (M)"], horizontal=True)
    gender = "F" if "Female" in gender_input else "M"
    
    available_categories = sorted(df_data['cat_clean'].unique()) if df_data is not None else ["OPEN", "OBC", "SC", "ST"]
    category = st.selectbox("Reservation Quota", options=available_categories, index=available_categories.index("OBC") if "OBC" in available_categories else 0)
    
    st.markdown("---")
    predict_btn = st.button("⚡ Run Predictor Engine", type="primary", use_container_width=True)
...
if predict_btn:
    if df_data is not None and knn_model is not None:
        ...
        top_combinations['Probability'] = (top_combinations['Count'] / total_count * 100).round(1)
        
        top_confidence = top_combinations.iloc[0]['Probability'] if not top_combinations.empty else 0.0

        st.markdown("<h3 style='color: #f8fafc; margin-bottom: 1rem;'>📊 Admission Diagnostic Summary</h3>", unsafe_allow_html=True)
        ...
        with tab_result:
            ...
            st.markdown("<br><h4 style='color: #38bdf8;'>🏆 Candidate College & Department Probabilities</h4>", unsafe_allow_html=True)
            
            for _, row in top_combinations.head(5).iterrows():
                col_name, col_bar = st.columns([3, 2])
                with col_name:
                    st.markdown(f"**{row['Institute Name']}**  \n<span style='color:#818cf8; font-size:0.9rem;'>{row['Course Name']}</span>", unsafe_allow_html=True)
                with col_bar:
                    st.progress(float(row['Probability'] / 100), text=f"{row['Probability']}% Probability")
        ...
        with tab_analytics:
            fig_gauge = go.Figure(go.Indicator(
                ...
                title={'text': "Score Competitiveness Index", 'font': {'size': 16, 'color': '#cbd5e1'}},
                gauge={
        ...
        with tab_benchmark:
            ...
            with col_chart2:
                dept_counts = matched_df['Course Name'].value_counts().head(5).reset_index()
        ...
        st.warning("⚠️ Dataset `CAP_Seat_Allocation (v2).xlsx - CAP I - Maharashtra (MHTCET) (1).csv` not found in working directory.")
...
st.divider()
st.caption("College Predictor Analytics Engine • Executive Data Science Dashboard")
