    ...
    if predict_btn:
        if model is not None:
            try:
                ...
                st.error(f"Error during prediction: {e}")import os
import pickle
import numpy as np
import pandas as pd
import streamlit as st
...
# App Header
st.markdown(
    '<div class="main-header">🎓 MHT-CET Admission Predictor</div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="sub-header">Predict potential college & department allocations based on MHT-CET cutoff trends</div>',
    unsafe_allow_html=True,
)
...
with col2:
    ...
    if predict_btn:
        if model is not None:
            try:
                # Prepare input DataFrame matching model feature names
                input_data = pd.DataFrame(
                    {
                        "MHTCET Percentile": [percentile],
                        "Gender": [gender],
                ...
                st.info(
                    "Tip: Verify that feature column names and encoding (e.g., One-Hot or Label Encoding) match what your model was trained on."
                )
        else:
    ...
    if df is not None:
        ...
        if percentile_col:
            filtered_df = df[
                (df[percentile_col] <= percentile + 2.0)
                & (df[percentile_col] >= percentile - 5.0)
            ]
        else:
            filtered_df = df

        st.write(
            f"Displaying top matching records near **{percentile} percentile**:"
        )
